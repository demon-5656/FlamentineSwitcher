...
if ! printf '%s' "$STATE_CONTENT" | grep -F "\"x11_text_target\": \"ru\"" >/dev/null 2>&1; then
  echo "Expected persisted per-app state was not written" >&2
...
if [ "$RESTORED_LAYOUT" != "ru" ]; then
  echo "Unexpected remembered per-app layout after restart: '$RESTORED_LAYOUT'" >&2
  echo "State content: $STATE_CONTENT" >&2
...
printf "E2E_OK app=x11_text_target layout=%s\n" "$RESTORED_LAYOUT"
